...
export default function Benchmark1() {
  ...
  const [result, setResult] = useState<{ frames: number; ms: number; renders: number } | null>(null)
    ...
    const tick = () => {
      ...
      if (rendersViewRef.current) {
        ...
        if (rendersRowsViewRef.current) {
          rendersRowsViewRef.current.textContent = `Rows re-rendered this frame: ${perFrame}/${rowsCount}`
        }
        if (rendersPercentViewRef.current) {
          ...
          const pct = Math.min(100, Math.round((avgPerFrame / rowsCount) * 100))
  ...
  const start = () => {
    ...
    const scheduleStop = () => {
      ...
      setResult({ frames, ms: Math.round(t1 - t0), renders: totalRenders })
  ...
  return (
    <div style={{ marginTop: 24 }}>
      ...
      <div className="section-card">
        ...
        {result ? <div style={{ fontSize: 12, color: '#333', marginTop: 4 }}>frames: {result.frames} • time: {result.ms} ms • renders: {result.renders}</div> : null}
      </div>
